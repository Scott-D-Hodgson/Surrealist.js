...
(function() {
    ...
    Surrealist._SortItemKeys = function(list, attribute, transitions) {
        ...
        if (Surrealist._ListExists(list, transitions)) {
            ...
            var sorted = itemDetails.sort((a, b) => { return a['sort'] - b['sort']});
            itemKeys = sorted.map((a) => a['id']);
        }
        return itemKeys;
    }
    ...
    Surrealist.RemoveByIds = function(ul, ids) {
        ...
        var itemsToKeep = itemKeys.filter((a) => { return !ids.includes(a) });
        var itemsToRemove = itemKeys.filter((a) => { return ids.includes(a) });
